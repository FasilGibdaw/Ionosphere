...
from numpy import mat, cos, sin, arctan, sqrt, pi, arctan2, e
import numpy as np
...
def ven2ecef(lla, ven):
    ...
    X = -sin(refLong)*ven[1] - cos(refLong)*sin(phiP) * \
        ven[2]+cos(refLong)*cos(phiP)*ven[0]+Xr
    Y = cos(refLong)*ven[1] - sin(refLong)*sin(phiP) * \
        ven[2]+cos(phiP)*sin(refLong)*ven[0]+Yr
...
def test_coord():
    ...
    print(ecef2lla(lla2ecef(np.array([lat, lon, alt]))))

    xyz = np.array([[-3197773.77194971,  -563853.79419661, -5587079.67459298]])
    print("\nlla2ecef( ecef2lla([%3.6e, %3.6e, %3.6e]) ) =" % tuple(xyz[0]))
    print(lla2ecef(ecef2lla(xyz)))

    lla = np.array([0, 0, 300e3])
    ven = np.array([0, 0, 200e3])
    print("\ntesting ven2ecef()...")
